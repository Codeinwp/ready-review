<div class="six columns" id="sidebar">
	<?php		
		$topprods = cwp('topprods');		
		if (isset($topprods) && $topprods == 'topprods_show') : 	
	?>
			<div class="widget">
				<?php 
				$theheading = cwp('topprods_heading');
				$thecat = cwp('topprods_cat'); 
				$thenum_temp = cwp('topprods_num');
				...
				switch ($thenum_temp) {
				 ...
				 $infotext = cwp('info_text');;
				 $rating = cwp('affiliate_rating');
				 $rank = cwp('affiliate_rank');
				 $orderby = cwp('orderby');
				...
				<?php while ($topposts_query->have_posts()) : $topposts_query->the_post(); ?>
					...
					<div class="row topprod">
						...
						<div class="nine columns topprodright">
							<div class="heading"><strong><a href="<?php the_permalink(); ?>" rel="nofollow"><?php the_title(); ?></a></strong></div>
							<div class="rating">
							<?php if($rating) { ?>
								<img src="<?php echo get_template_directory_uri(); ?>/images/<?php echo $rating; ?>-star.png" border="0" />
							<?php } ?>
							</div>
							<div class="readmore"><a href="<?php the_permalink(); ?>" class="readreview"><?php if($infotext) { echo $infotext;?><?php } else { _e('Read Review','cwp'); } ?></a></div>
						</div>
