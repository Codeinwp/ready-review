...
function cwp_theme_setup() {
	...
	register_nav_menus(
        array(
            'main-menu' => __( 'Main','cwp' )
        )
    );
	
	add_image_size('med-thumb', 150, 150, true );	
	add_image_size('small-thumb', 100, 100, true );
	add_image_size('single-thumb', 300, false );		require( get_template_directory() . '/admin/functions.php' );  
}
...
function cwp_scripts() {
	...
	if ( is_singular() ) 
		...
		wp_enqueue_script( 'cwp_modernizr', get_template_directory_uri() . '/js/libs/modernizr-2.0.6.min.js', array("jquery"), '20120206', true );
		wp_enqueue_script( 'cwp_customscript', get_template_directory_uri() . '/js/customscript.js', array("jquery"), '20120206', true );
	 
		wp_enqueue_script( 'cwp_gumby', get_template_directory_uri() . '/js/libs/gumby.min.js', array("jquery"), '20120206', true );
		wp_enqueue_script( 'cwp_plugins', get_template_directory_uri() . '/js/plugins.js', array("jquery"), '20120206', true );
		wp_enqueue_script( 'cwp_responsinav', get_template_directory_uri() . '/js/jquery.responsinav.js', array("jquery"), '20120206', true );
...
{
	add_meta_box( 'my-meta-box-id', 'Affiliate Settings', 'cwp_meta_box_cb', 'post', 'normal', 'high' );
}

function cwp_meta_box_cb( $post )
{
...
add_action( 'save_post', 'cwp_meta_box_save' );
function cwp_meta_box_save( $post_id )
{
 ...
 */
add_action('admin_init','cwp_optionscheck_change_santiziation', 100);
function cwp_optionscheck_change_santiziation() {
    remove_filter( 'of_sanitize_textarea', 'of_sanitize_textarea' );
    add_filter( 'of_sanitize_textarea', 'cwp_custom_sanitize_textarea' );
}
function cwp_custom_sanitize_textarea($input) {
